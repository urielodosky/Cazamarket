...
export default function NotificacionesPage() {
  ...
  const getIconForType = (type: string) => {
    ...
    switch (type) {
      ...
      case 'global':
        return <div className={baseClasses + "bg-orange-500/10 text-orange-500"}><svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg></div>;
      case 'system':
      default:
        return <div className={baseClasses + "bg-gray-500/10 text-gray-500"}><svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg></div>;
    }
  };
            ...
            {isLoading ? (
              ...
              notifications.map(n => (
                <button
                  ...
                  className={`w-full text-left flex flex-col gap-2 p-4 rounded-xl transition-all duration-200 border ${
                    selectedNotif?.id === n.id ? 'bg-[var(--color-bg-surface-elevated)] shadow-lg border-[var(--color-primary)]/50' : 
                    n.is_read ? 'hover:bg-[var(--color-bg-surface-elevated)] border-[var(--color-border)]' : 'bg-[var(--color-primary)]/5 hover:bg-[var(--color-primary)]/10 border-[var(--color-primary)]/20'
                  }`}
                >
                  <div className="flex justify-between items-center w-full">
                    <span className={`text-[0.65rem] uppercase tracking-wider font-bold px-2 py-1 rounded-md ${n.type === 'global' ? 'bg-orange-500/20 text-orange-500' : 'bg-gray-500/20 text-gray-500'}`}>
                      {n.type === 'global' ? 'Aviso General' : 'Aviso Individual'}
                    </span>
                  ...
                  <div className="flex gap-4 items-start w-full">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                      <div className="flex justify-between items-start mb-1">
                        <h3 className={`text-sm line-clamp-2 pr-2 ${n.is_read ? 'text-[var(--color-text-main)] font-medium' : 'text-[var(--color-primary)] font-bold'}`}>
                          {n.title}
                        </h3>
                      </div>
                  ...
                  <div className="flex items-center gap-4 mb-6 pb-6 border-b border-[var(--color-border)]">
                    ...
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        <h2 className="text-2xl font-bold text-[var(--color-text-main)]">{selectedNotif.title}</h2>
                        <span className={`text-[0.65rem] uppercase tracking-wider font-bold px-2 py-1 rounded-md ${selectedNotif.type === 'global' ? 'bg-orange-500/20 text-orange-500' : 'bg-gray-500/20 text-gray-500'}`}>
                          {selectedNotif.type === 'global' ? 'Aviso General' : 'Aviso Individual'}
                        </span>
                      </div>
